...
def train_and_predict(input_images):
    #create a MNIST folder with the MNIST dataset if necessary
    ...
    #use 1000 batches with a size 100 each to train our net
    for i in range(1000):
        batch_xs, batch_ys = mnist.train.next_batch(100)
        #run the train_step function the given image values (x) and the real output (y_)
    ...
    #create an a array where can store our 1 pictures
    ...
    for no in input_images:
        ...
        colsPadding = (int(math.cell((28-cols)/2.0)), int(math.floor((28-cols)/2.0)))
        rowsPadding = (int(math.cell((28-cols)/2.0)), int(math.floor((28-cols)/2.0)))
